...
RegisterNetEvent('qb-pawnshop:client:openMenu', function()
    if Config.UseTimes then
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed 
        or  GetClockHours() >= Config.TimeOpennew and GetClockHours() <= Config.TimeClosednew
        then
            local pawnShop = {
                {
                    header = Lang:t('info.title'),
           ...
           --[[ if not isMelting then
            ...
            QBCore.Functions.Notify('Si děláš prdel, během dne?! Přijď mezi 22:00 a 00:00 nebo ráno mezi 2:00 a 5:00h.')
        end
    else
        local pawnShop = {
            {
...
RegisterNetEvent('qb-pawnshop:client:pawnitems', function(item)
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            ...
            TriggerServerEvent('sellnpc:log')
        else
